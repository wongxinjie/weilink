...
def chat_page(request, pid):
	...
	black_query = Blacklist.objects.filter(wluserid=request_user.id, blackerid=authuser.id)
	if black_query or request_user.id == authuser.id:
		return HttpResponseRedirect("/people/home")

	Letter.objects.filter(senderid=request_user.id, receiverid=authuser.id, been_read=False).update(been_read=True)
	sended_letter_query = Letter.objects.filter(senderid=authuser.id, receiverid=request_user.id, sender_delete=False)
	received_letter_query = Letter.objects.filter(senderid=request_user.id, receiverid=authuser.id, receiver_delete=False)
...
def get_newest_letter(request):
	...
	suser = speople.user
	letter_query = Letter.objects.filter(senderid=suser.id, receiverid=authuser.id, been_read=False)
